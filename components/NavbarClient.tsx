...
export default function NavbarClient({ session, logoutAction }: NavbarClientProps) {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 w-full flex h-20 items-center justify-between px-4 sm:px-8 lg:px-12 select-none transition-all duration-500 ease-in-out ${
        isHome
          ? "bg-transparent border-b border-transparent shadow-none"
          : "bg-[#0f1a1b]/85 backdrop-blur-md border-b border-transparent shadow-[0_4px_25px_rgba(0,0,0,0.4)]"
      }`}
    >
      <div className="mx-auto flex w-full max-w-[98%] items-center justify-between relative">
        ...
        {/* Right Segment: Links, Search, REC dot & Auth (Desktop) */}
        <div className="hidden md:flex items-center gap-4 lg:gap-6 z-10">
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/recommend"
              className="relative group px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest text-[#FAF6E8] bg-gradient-to-r from-[#B58863]/15 to-[#d4a87c]/10 border border-[#B58863]/30 hover:border-[#d4a87c]/60 shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] hover:shadow-[0_0_15px_rgba(181,136,99,0.2)] hover:scale-[1.02] active:scale-[0.98] transition-all duration-300 flex items-center gap-1.5"
            >
              <span className="w-1 h-1 rounded-full bg-[#d4a87c] animate-pulse" />
              <span>Match Finder</span>
            </Link>

            <Link
              href="/profile"
              className="relative group px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest text-[#D3C3B9] bg-white/5 border border-white/10 hover:border-[#B58863]/40 hover:text-white hover:shadow-[0_0_15px_rgba(255,255,255,0.05)] hover:scale-[1.02] active:scale-[0.98] transition-all duration-300"
            >
              Profile
            </Link>
          </div>
          ...
          <form 
            action="/search" 
            method="GET" 
            className={`relative flex items-center rounded-full px-3.5 py-1.5 transition-all duration-300 ${searchFormClass}`}
          ...
          >
            <input
              ...
              className={`w-36 sm:w-44 md:w-52 lg:w-60 bg-transparent text-[10px] focus:outline-none transition-all duration-300 font-mono ${searchInputClass}`}
              required
              autoComplete="off"
            />
            <button type="submit" className={`cursor-pointer pl-1.5 transition-colors ${searchBtnClass}`}>
          ...
          <div className="hidden lg:flex items-center gap-1.5 font-mono text-[8px] tracking-widest text-red-500/80 bg-red-550/5 border border-red-200 px-2 py-0.5 rounded-full select-none">
          ...
          {session?.user ? (
            <div className="group relative flex items-center">
              ...
              <div className="absolute right-0 top-full mt-3.5 w-52 rounded-xl bg-[#0e1a1c]/95 backdrop-blur-xl border border-white/[0.08] py-2.5 shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 transform origin-top-right translate-y-1 group-hover:translate-y-0">
                ...
                <form action={logoutAction} className="relative z-10 border-t border-white/[0.07] mt-1.5 pt-1.5">
                  <button
                    type="submit"
            ...
            <div className="flex items-center">
              <Link
                href="/login"
                className="text-xs font-semibold bg-gradient-to-r from-[#B58863] to-[#d4a87c] text-[#0f1a1b] px-4.5 py-1.5 rounded-full transition-all shadow-md shadow-[#B58863]/20 hover:shadow-[#B58863]/40 hover:opacity-90 font-mono tracking-wider"
              >
                Sign In
              </Link>
            </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-col gap-2 pt-2">
                    ...
                    <form action={logoutAction} className="pt-2">
                      <button
                        type="submit"
                        className="block w-full text-center px-4 py-2.5 text-xs bg-red-50 border border-red-100 text-red-500 rounded-xl hover:bg-red-100/50 transition-all font-bold cursor-pointer"
                      >
                        Sign Out
                      </button>
                    </form>
